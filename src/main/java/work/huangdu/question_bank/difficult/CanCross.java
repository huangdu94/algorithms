...
/*    private int n;
    ...
    private boolean go(int k, int cur) {
        if (memo.contains(new int[] {k, cur})) { return false; }
        if (cur == n - 1) { return true; }
        for (int step = k - 1; step <= k + 1; step++) {
            int start = stones[cur], next = cur + 1;
            while (next < n && stones[next] - start < step) {
                next++;
            }
            if (next == n) { return false;}
            if (stones[next] - start == step) {
                if (go(step, next)) { return true; } else {
                    memo.add(new int[] {k, cur});
                }
            }
        }
        return false;
