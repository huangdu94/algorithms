...
public class MinOperations {
    public int minOperations(int[] target, int[] arr) {
        int n = target.length, len = 0;
        Map<Integer, Integer> idMap = new HashMap<>();
        for (int i = 0; i < n; i++) {
            idMap.put(target[i], i);
        }
        for (int num : arr) {
            Integer id = idMap.get(num);
            if (id != null) {
                arr[len++] = id;
            }
        }
        return n - lengthOfLIS(Arrays.copyOf(arr, len));
    }
    ...
    public static void main(String[] args) {
        ...
        System.out.println(mo.minOperations(target, arr));
    }
}
