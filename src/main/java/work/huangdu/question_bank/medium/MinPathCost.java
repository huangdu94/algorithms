...
public class MinPathCost {
    public int minPathCost(int[][] grid, int[][] moveCost) {
        ...
        for (int k = m - 2; k >= 0; k--) {
            int[] cur = grid[k], next = grid[k + 1];
            for (int i = 0; i < n; i++) {
                int min = Integer.MAX_VALUE;
                for (int j = 0; j < n; j++) {
                    min = Math.min(min, next[j] + moveCost[cur[i]][j]);
                }
                cur[i] += min;
            }
        }
        for (int cost : grid[0]) {ans = Math.min(ans, cost);}
        return ans;
    }
    ...
    public static void main(String[] args) {
        ...
        System.out.println(mpc.minPathCost(grid, moveCost));
    }
}
