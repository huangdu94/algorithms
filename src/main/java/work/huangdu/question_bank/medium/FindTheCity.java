...
public class FindTheCity {
    public int findTheCity(int n, int[][] edges, int distanceThreshold) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            distance[i] = 0;
            int count = 0;
            while (!queue.isEmpty() && count <= minCount) {
                int cur = queue.poll();
                for (int next = 0; next < n; next++) {
                    int nextDistance = distance[cur] + graph[cur][next];
                    if (nextDistance < distance[next] && nextDistance <= distanceThreshold) {
                        if (distance[next] == Integer.MAX_VALUE) {count++;}
    ...
    public static void main(String[] args) {
        ...
        System.out.println(ftc.findTheCity(n, edges, distanceThreshold));
    }
}
